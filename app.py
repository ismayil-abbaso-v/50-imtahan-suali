import streamlit as st import re import random from docx import Document from io import BytesIO

def parse_docx(file): doc = Document(file) sual_nusunesi = re.compile(r"^\s*\d+[.)]\s+") variant_nusunesi = re.compile(r"^\s*[A-Ea-e])\s+(.*)")
...
def create_shuffled_docx_and_answers(secilmis): yeni_doc = Document() cavablar = []
...
st.set_page_config(page_title="Test Qarışdırıcı", page_icon="📄") st.title("📄 Word test suallarını qarışdır") st.markdown(""" Yüklə .docx sənədini → 50 təsadüfi sual seçilsin → Variantlar qarışdırılsın → Cavab siyahısı çıxarılsın ✅ """)

uploaded_file = st.file_uploader("Word (.docx) sənədini seç", type="docx")

if uploaded_file: suallar = parse_docx(uploaded_file) if len(suallar) < 5: st.error("Faylda kifayət qədər uyğun sual tapılmadı. Formatı yoxla.") else: secilmis = random.sample(suallar, min(50, len(suallar))) yeni_doc, cavablar = create_shuffled_docx_and_answers(secilmis)
